...
Product Introduction: We have been tasked by Frito Lay to develop a model to predict employee attrition and employee salary. We have also been tasked with finding the 
...
Our first model is a k-NN with all variables thrown into the model. The k with the highest mean accuracy and sensitivity had both values below 60%, so this model would not be used. 
```{r}
...
According to our model, the 3 most important factors in determining attrition are if someone works overtime, overall job satisfaction and job role, with manager, manufacturing director and sales representative less likely to leave than the rest of your employees. 
```{r}
summary(stepModel)  
Predz = predict(stepModel, newdata = DDStest, type = 'response')
Probs = ifelse(Predz > 0.45, 'Yes', 'No')
CM = confusionMatrix(table(Probs, DDStest$Attrition), positive = 'Yes')
